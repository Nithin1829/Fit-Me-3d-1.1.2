...
// // Middleware
// // app.use(cors());
// // app.use(bodyParser.json({ limit: '50mb' })); // To handle large GLTF data
// // Middleware
// app.use(cors());
...
// // User model schema (including measurements and GLTF link)
...
//     const gltfFilePath = `./uploads/${gltfFileName}`;
...
// //     res.status(200).json({ measurements: user.measurements, gltfFile: user.gltfFile });
// //   } catch (error) {
...
//     // Correct the path for frontend access
//     const publicGltfPath = `/uploads/${user.gltfFile.split('/').pop()}`;
